 ...
 *Created: May 8, 2007
 *
 * this code is explained here http://www.dreamincode.net/forums/topic/27950-steganography/
 */
...
public class LSBEncDec
{
    ...
    public String decode(String absolutePath)
    {
        ...
        try
        {
            ...
            System.out.println(String.format("Decoding %s", absolutePath));
            BufferedImage image  = user_space(getImage(absolutePath));
            System.out.println("Image bufferd");
            byte[] imageBytes = get_byte_data(image);
            System.out.println("Trabsoformed bufferd image on bytes");
            System.out.println("Displaying the buffered image");
            for(int i=0; i < imageBytes.length; i++ ){
                System.out.println("    "+imageBytes[i]);
            }
            decode = decode_text(imageBytes);
            if(decode == null ) {
                return null;
            }
            System.out.println("Retrieved hidden text from the image");
    ...
    private byte[] decode_text(byte[] image)
    {
        ...
        {
            System.out.println(String.format(" image[%s] = %s", i, image[i]));
            length = (length << 1) | (image[i] & 1);
        }
        System.out.println(String.format("    hidden text length: %s", length));
        if(length <= 0) return null;
        byte[] result = new byte[length];

        //loop through each byte of text
        System.out.println("    looping through each byte of text");
